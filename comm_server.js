...
med_io.on('connection', function (socket) {
    ...
    var sessId = socket.handshake.headers.cookie.split("PHPSESSID=")[1].split(";")[0];
    if (!sessId) {
...
var sendText = function (textData, socket) {
    var msg = { userId: 0, serverId: textData.Entry_ID, editId: 0, num_nodes: textData.Num_Style_Nodes };
            ...
            my_sql_pool.getConnection(function (err, connection) {
                if (!err) {
                    if (typeof (data.localId) != 'undefined' && data.num_points && data.colour) {
                        connection.query('USE Online_Comms');
                        connection.query('INSERT INTO Whiteboard_Space(Room_ID, User_ID, Local_ID, Edit_Time, Num_Control_Points, Colour, Size) VALUES(?, ?, ?, CURRENT_TIMESTAMP, ?, ?, ?)', [boardConnData[socket.id].roomId, boardConnData[socket.id].userId, data.localId, data.num_points, data.colour, data.size], function (err, result) {
                            if (err) {
                ...
                my_sql_pool.getConnection(function (err, connection) {
                    if (!err) {
                        ...
                        connection.query('UPDATE Control_Points SET X_Loc = (X_Loc + ?), Y_Loc = (Y_Loc + ?) WHERE Entry_ID = ?', [data.x, data.y, data.serverId], function (err, rows) {
                            if (!err) {
                                var msg = { serverId: data.serverId, x: data.x, y: data.y };
                        ...
                        if (err) {
                            ...
                            if (rows[0]) {
                                var retData = {
                                    ...
                                    colour: rows[0].Colour, size: rows[0].Size
                                };
                ...
                my_sql_pool.getConnection(function (err, connection) {
                    if (!err) {
                        ...
                        connection.query('UPDATE Text_Space SET Width = ?, Height = ? WHERE Entry_ID = ?', [data.width, data.height, data.serverId], function (err, rows) {
                            if (!err) {
                                var msg = { serverId: data.serverId, width: data.width, height: data.height };
